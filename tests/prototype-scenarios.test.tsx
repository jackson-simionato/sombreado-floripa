...
describe("prototype scenarios", () => {
  test.each(scenarioExpectations)("renders %s", async (prototypeScenarioId, expectedHeading) => {
  ...
  test.each([
    ["location-request", "De que lado sentar?", "Usar minha localização", "Procurar linha manualmente"],
    ["location-denied", "Localização desativada", "Procurar linha manualmente", "Tentar localização de novo"],
    ["routes-none-nearby", "Não encontrei linhas perto de você", "Procurar linha manualmente", "Tentar localização de novo"],
    ["manual-search-empty", "Nenhuma linha encontrada", "Buscar de novo", "Usar minha localização"],
    ["direction-unavailable", "Não é possível confirmar o sentido", "Trocar linha", "Procurar linha manualmente"],
    ["confirmation", "Confirme sua linha", "Confirmar esta linha", "Trocar sentido"],
    ["confirmation-fallback", "Confirme sua linha", "Confirmar mesmo assim", "Trocar sentido"],
    ["advice-withheld", "Não é possível recomendar agora", "Trocar linha", "Tentar de novo"],
    ["error-nearby-routes", "Algo deu errado", "Tentar de novo", "Procurar linha manualmente"],
    ["error-directions", "Algo deu errado", "Tentar de novo", "Procurar linha manualmente"],
    ["error-geometry", "Algo deu errado", "Tentar de novo", "Procurar linha manualmente"],
    ["error-advice", "Algo deu errado", "Tentar de novo", "Procurar linha manualmente"]
  ] as const)(
    "renders contractual actions for %s",
    async (prototypeScenarioId, expectedHeading, primaryActionLabel, fallbackActionLabel) => {
      render(<HomePageApp prototypeScenarioId={prototypeScenarioId} />);

      expect(await screen.findByRole("heading", { name: expectedHeading })).toBeInTheDocument();
      expect(screen.getByRole("button", { name: primaryActionLabel })).toBeInTheDocument();
      expect(screen.getByRole("button", { name: fallbackActionLabel })).toBeInTheDocument();
    }
  );
  ...
  test("scenario switcher reaches every mocked prototype state from the home route", async () => {
    ...
    expect(optionLabels).toEqual(prototypeScenarios.map((scenario) => scenario.label));
