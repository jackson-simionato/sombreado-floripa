...
Read these files before domain-sensitive work:
...
- `docs/adr/` when it exists and contains decisions relevant to the task.

If `docs/adr/` does not exist, proceed silently. The `/grill-with-docs` workflow creates ADRs lazily when a hard-to-reverse decision needs one.
...
If a recommendation contradicts an existing ADR, surface the conflict explicitly before proceeding.
